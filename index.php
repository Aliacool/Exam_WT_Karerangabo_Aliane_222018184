<!DOCTYPE html>
<html>

<head>
    <title>Online Personal Counselling</title>
    <link href="Home_style.css" rel="stylesheet">

</head>

<body>
    <!--Navigation-->
    <div>
        <ul id="navi-bar">
            <img id="logo" src="images/logo.png" alt="Personal Online Counselling">
            <li><a class="nav-item" href="Service.php">Service</a></li>
            <li><a class="nav-item" href="Counsellors.php">Counsellors</a></li>
            <li><a class="nav-item" href="Invite.php">Invite</a></li>
            <li><a class="nav-item" href="My account.php">Login</a></li>
            <li><a class="nav-item" href="Registation.php">Register</a></li>
            <li><a class="nav-item" href="About.php">About us</a></li>

        </ul>
    </div>
    <!--Body-->

    <div id="back-img">
        <div id="home">

            <p id="home-para3">SOLUTIONS ARE HERE</p>
            <br>
            <p id="home-para2">WELCOME</p>
            <br>
            <p class="home-para">ONLINE PERSONAL COUNSELLING</p>

        </div>
    </div>
    <div id="home2">
        <div class="home2-para">
            <p class="para">Hasn't Planned Your Life Well Yet ?</p>
        </div>
        <div class="home2-para">
            <p class="para">Fear of Exam ?</p>
        </div>
        <div class="home2-para">
            <p class="para">Panic Hearing Problems ?</p>
        </div>
        <div class="home2-para">
            <p class="para">Are You Hot Tempered ?</p>
        </div>
    </div>
	<!-- Examples-->
    <div id="img2">
        <img src="images/1.jpg" width="24%">
        <img src="images/2.jpg" width="24%">
        <img src="images/3.jpg" width="24%">
        <img src="images/4.png" width="24%">
    </div>
    <div id="backimg2">
        <p id="backpara">Today and We Will Ensure a Bright Future For You ....</p>
        <p id="backpara2">
            Then Why Are You Still Waiting For ?

        </p>
    </div>

    <!--footer-->
    <div id="footer-background">

        <!--address-->
        <div id="foot-addr">
            <h3 class="address">Personal Online Counselling </h3>
            <h3 class="address">Karerangabo Aliane.</h3>
            <h2 class="address">T.P-078855253</h3>
        </div>
        <!--social media-->
        <div id="social-media">
            <h3>Follow us :</h3>
            <P>
                <a href=" "><img src="images/fac.png" width="100 "></a>
                <a href=" "><img src="images/ins.png" width="100 "></a>
                <a href=" "><img src="images/twi2.png" width="100 "></a>
            </P>
        </div>
    </div>

</body>

</html>